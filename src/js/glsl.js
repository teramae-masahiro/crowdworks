...
import skyImage from "@imgs/sky.jpg";
...
const animate = () => {
  ...
  camera.position.x  =  Math.sin(elapsedTime * .17) * 3.;
  camera.position.z  =  Math.cos(elapsedTime * .17) * 3.;

  camera.lookAt(Math.cos(elapsedTime),Math.sin(elapsedTime) * .4,Math.sin(elapsedTime) * .4);
